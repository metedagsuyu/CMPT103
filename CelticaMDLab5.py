...
def displayText(board):
    boardFormat = []
    boardFormat.append("".join(board[24:29]+ board[:4]))
    #print("".join(board[24:29]+ board[:4]))    
    
    for repeat in range(3):
        #print("".join([board[24]]+ [" "] *7 + [board[0]]))
        boardFormat.append(("".join([board[24]]+ [" "] *7 + [board[0]])))
                     
    #print("".join([board[16]] + [" "]*3 + [board[32]] + [" "]*3 + [board[0]]))
    boardFormat.append("".join([board[16]] + [" "]*3 + [board[32]] + [" "]*3 + [board[0]]))  
    
    for repeat in range(3):
        #print("".join([board[16]] + [" "]*7 + [board[8]]))
        boardFormat.append("".join([board[16]] + [" "]*7 + [board[8]]))
        
    #print("".join(board[16:20]+ board[8:13])) 
    boardFormat.append("".join(board[16:20]+ board[8:13]))
